...
const statusFilterOptions = ["all", "active", "on_hold", "completed", "cancelled"];
...
export default function ProjectsPage() {
  ...
  function openEditModal(project: Project) {
    ...
    setFormData({
      ...
      budgetHours: project.budgetHours
        ? String(parseFloat(project.budgetHours))
        : "",
      budgetAmount: project.budgetAmount
        ? String(parseFloat(project.budgetAmount))
        : "",
  ...
  // Pre-fill hourly rate from client
    ...
    setFormData((prev) => {
      ...
      return {
        ...
        hourlyRate:
          ...
          !prev.hourlyRate &&
          selected?.defaultHourlyRate
            ? String(parseFloat(selected.defaultHourlyRate))
            : prev.hourlyRate,
      };
    });
  ...
  async function handleSaveProject(e: React.FormEvent) {
    ...
    if (!formData.clientId && !editingProject) errs.clientId = "Client is required.";
    if (
    ...
    if (
      ...
      (isNaN(Number(formData.budgetHours)) || Number(formData.budgetHours) < 0)
    )
      errs.budgetHours = "Budget hours must be a positive number.";
            ...
            {loading && projects.length === 0 ? (
              ...
              projects.map((project) => {
                const budgetHours = project.budgetHours
                  ? parseFloat(project.budgetHours)
                  : 0;
                // No time tracking yet — budget percent will be 0 until time entries exist
                const budgetPercent = 0;
                const isOverdue =
                  project.deadline &&
                      ...
                      <div className="flex items-center gap-4 pt-3 mt-3 border-t border-gray-100 text-xs text-gray-500">
                        ...
                        <span className="ml-auto font-mono">
                          {project.billingType === "hourly" && project.hourlyRate
                            ? formatCurrency(parseFloat(project.hourlyRate)) +
                              "/hr"
                            : project.fixedPrice
          ...
          <div className="relative bg-white rounded-lg shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto p-6 animate-fade-in">
            ...
            <form onSubmit={handleSaveProject} className="space-y-5">
              ...
              <div>
                ...
                {!clientsLoading && clients.length === 0 && !editingProject && (
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <button
                    type="button"
                    onClick={() =>
                      setFormData({ ...formData, billingType: "hourly" })
                    }
                  ...
                  <button
                    type="button"
                    onClick={() =>
                      setFormData({ ...formData, billingType: "fixed_price" })
                    }
                    className={`flex-1 py-2 px-3 text-sm font-medium rounded-md border transition-colors ${
                      formData.billingType === "fixed_price"
                        ? "bg-primary-50 border-primary-500 text-primary-700"
